...
1. ~~接続時にCFG-VALSETで定期出力を有効化~~ → **無効化に変更（Session 145）**
2. `GET /api/gnss-state` を新設し、全データを1回で取得（ポーリング方式）
3. FE側のポーリングを統合APIに集約

### Session 145以降の変更

**定期出力を無効化に変更した理由**:
- 定期出力が有効だと、ポーリング応答と混在してパースエラーが発生
- 統合APIは順次ポーリングで動作するため、定期出力は不要

**Layer指定**:
- Session 149: `Layer::Bbr` に変更（不揮発性にするため）
- Session 151: `Layer::RamAndBbr` に変更（BBRのみでは即座に有効にならない）

## 実装内容

### 完了

- CFG-VALSET生成関数 (`set_periodic_output`, `disable_periodic_output`)
- 接続時に定期出力を無効化
- 統合API (`GET /api/gnss-state`) 実装
- FE側のポーリング集約
- UART1用キー追加（実機はUART1接続のため）
...
| 2026-03-12 | 定期出力を無効化に変更 | Session 145 |
| 2026-03-12 | UART1用キー追加、Layer::Bbr | Session 149 |
| 2026-03-12 | Layer::RamAndBbrに変更（BBRのみでは即座に有効にならない） | Session 151 |
