...
**修正履歴**:
- 2026-03-19（Session 271）: アーカイブ運用の記載を修正（Linearの仕様誤解を訂正）
...
**Linearのアーカイブは自動実行**（修正: Session 271）:
- Issueを完了状態（Done）にすると、設定されたアーカイブ期間後に自動アーカイブ
- 手動でのアーカイブ操作は不可（Linear仕様）
- アクティブissue数を抑制するには、Issue完了（Done状態に変更）を徹底
- アーカイブしたissueは無制限（検索・参照可能）

**Issue完了の方法**:
- `issueUpdate` Mutationで`stateId`を完了状態（Done）に変更
- 詳細は `~/.claude/skills/linear-management/SKILL.md` 参照
...
1. **Issue完了の徹底が必要**: アクティブissue数を抑制するため、Issue完了（Done状態）を徹底する必要がある
...
| リスク | 対策 |
|--------|------|
| アクティブissueが予想以上に増える | 150件で相談開始、Issue完了を徹底 |
| Issue完了忘れで上限到達 | セッション開始時に自動監視・警告 |
