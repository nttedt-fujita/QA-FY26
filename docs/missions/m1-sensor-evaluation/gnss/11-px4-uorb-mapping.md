...
|-----------|-----|--------------|------|
| count | uint8 | — | 衛星数 |
| svid | uint8[20] | UBX-NAV-SAT | 衛星ID |
| used | uint8[20] | UBX-NAV-SAT | 使用中フラグ |
| elevation | uint8[20] | UBX-NAV-SAT | 仰角（deg） |
| azimuth | uint8[20] | UBX-NAV-SAT | 方位角（deg） |
| snr | uint8[20] | UBX-NAV-SAT | **C/N0（dBHz）** |

**注意**: L1/L2の区別がない可能性あり。**要調査**。
...
| ジャミング状態 | ✅ 可能 | jamming_state |
| **受信感度（C/N0）** | ⚠️ 要確認 | satellite_info.snr（L1/L2区別なし？） |
| **L1/L2別C/N0** | ❓ 不明 | UBX-NAV-SIG相当の情報があるか要調査 |
| **スペアナデータ** | ❌ 不可 | UBX-MON-SPANはuORB化されていない |
| **TTFF** | ❓ 不明 | UBX-NAV-STATUS.ttffがuORB化されているか要調査 |

---

## 5. 直接UBX通信が必要な項目

以下はフライトログ経由では取得できない（可能性が高い）ため、F9Pとの直接通信が必要：

| 評価項目 | 理由 |
|----------|------|
| L1/L2別の受信感度（C/N0） | PX4がL1/L2を区別してuORB化していない可能性 |
| スペアナデータ | UBX-MON-SPANがuORB化されていない |
| 内部設定（FW/PROTVER/パラメータ） | UBX-CFG-*で直接読み取り |
