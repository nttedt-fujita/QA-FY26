...
出典: [Session 16 小板橋さんヒアリング](../session16/gnss-hearing-koitabashi-01.md)
...
[07-cross-sheet-findings.md](../../docs/missions/m1-sensor-evaluation/gnss/07-cross-sheet-findings.md) を確認すると、以下がExcelにはあるが叩き台の合格基準に反映されていない:
...
| 検証項目 | 旧叩き台 | 業界標準に基づく修正案 | 根拠 |
|---------|---------|---------------------|------|
| L1受信感度 | ≥25 dBHz | **≥30 dBHz** | トラッキング維持の閾値 |
| L2受信率 | GPS 50%以上 | 要確認 | L2閾値の明確な基準は見つからず |
| RTK FIX時間 | ≤3分 | **≤30秒〜60秒** | 一般的KPI。環境考慮で60秒も可 |
| 飛行中水平精度 | ≤1.0m | ≤1.0m（維持） | RTK機能確認としては妥当 |
| 飛行中衛星数 | ≥20 | ≥20（維持）または≥15 | 4系対応で20は達成可能 |

---

## 8. 残った疑問（小板橋さん・末永さんに確認）
...
| # | 質問 | 背景 |
|---|------|------|
| 1 | No.5の13 dBHzという値は、どの衛星・どの条件で測定されたか？ | 30 dBHz基準と比較するため |
| 2 | 現在の設計検証では、どのような判定基準を使っているか？ | 暗黙の基準があれば知りたい |
| 3 | L2受信率「GPS衛星の50%以上」は妥当か？ | L2基準が業界に見つからなかった |
| 4 | RTK FIX時間3分は現場運用の実態に基づくか？ | 業界標準は30秒だが、環境依存が大きい |

#### 叩き台に含まれていない項目に関する質問（追加）

| # | 質問 | 背景 |
|---|------|------|
| 5 | **TTFF（Cold/Warm/Hot Start）は設計検証で確認しているか？** | 一般的なGNSSテストで必須の項目 |
| 6 | **PDOPの閾値は設定しているか？** | 衛星配置が悪いと精度劣化 |
| 7 | Hot Start確認の手順が曖昧と記載があったが、確立したか？ | Excelにコメントあり |
| 8 | ファームウェアバージョンの確認はしているか？ | 内部設定の前提 |
| 9 | 小峰無線との内部設定合意は完了したか？ | Excelにコメントあり |

### 末永さんに確認すべきこと

| # | 質問 | 背景 |
|---|------|------|
| 1 | AirGrowの飛行制御でGNSSに要求される精度は？ | 1.0mで十分か、cm級が必要か |
| 2 | RTK FIXできない場合のフォールバック動作は？ | Float運用の可否 |
| 3 | 飛行中の最低衛星数は何個か？ | 20が妥当か、もっと少なくてよいか |
| 4 | 受信感度30 dBHzを下回ったらどうなるか？ | 実機での挙動確認 |
| 5 | **再捕捉時間（Reacquisition Time）の要件は？** | 遮蔽物通過後の復帰時間 |
| 6 | **ジャミング発生時のフェールセーフ動作は？** | 飛行試験でスパイク観測あり |
