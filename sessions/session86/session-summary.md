...
DB設計に以下の議論内容が十分反映されていない:

| 議論内容 | 要検討 |
|----------|--------|
| FWバージョンの多数派・はずれ値比較 | ロット単位で集計する設計が不明確 |
| 装置のFWバージョンを直接見れる | devicesにfw_versionカラム追加 or クエリ対応 |
| 検査項目5つ | item_nameの具体的な値を定義 |

詳細: [unified-db-schema.md](unified-db-schema.md) の「5. 未反映の議論内容」参照
