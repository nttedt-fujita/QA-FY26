...
| ファイル | 内容 |
|----------|------|
| `app/inspections/history/page.tsx` | 履歴再生画面（新規） |

**変更ファイル**:
| ファイル | 変更内容 |
|----------|----------|
| `app/inspections/outdoor/page.tsx` | スナップショット重複チェック追加、履歴リンク追加 |
| `app/inspections/page.tsx` | 履歴再生カード追加（3カラム化） |
| `lib/api.ts` | listOutdoorResults, getOutdoorSnapshots追加 |

**次セッション（Session 181）でやること**:
以下の要望を優先順位整理→調査→計画→実装の流れで対応

| # | 要望 | 備考 |
|---|------|------|
| 1 | シリアル番号表示 | どのGNSSレシーバーのデータか識別 |
| 2 | RF波形比較機能 | 複数検査のRF波形を並べて比較 |
| 3 | RF波形サンプルごと確認 | 本当に取れてるか調査必要 |
| 4 | L1 CNO MIN/MAX/平均 | MINだけにした根拠を再調査 |
| 5 | GNSS指定受信 | u-centerでは可能、UBX設定調査 |
| 6 | RF波形表示サイズ | 16:9〜4:3で大きめ表示 |
| 7 | 履歴からGNSS切り分け | 難しいかも（要調査） |
