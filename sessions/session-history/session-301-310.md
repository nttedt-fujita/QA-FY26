# Session 301-310

## Session 301 (2026-03-24)

**概要**: drawioのPhase番号を4フェーズ構成に更新 / ファイル整理

**重要な成果**:
- Phase 1-4の全drawioファイルを4フェーズ構成に更新
- ファイル名をフェーズ番号に合わせてリネーム
- Phase 2用drawioをsession297からコピー・整理
- スクショ保存（ユーザー側で実施）

**作成ファイル**:

| ファイル | 内容 |
|----------|------|
| session296/phase1-aggregation-automation.drawio | Phase 1: 集計業務の自動化 |
| session296/phase2-inventory-recording.drawio | Phase 2: 入出庫の自動記録 |
| session296/phase3-manufacturer-visibility.drawio | Phase 3: 委託先在庫の見える化 |
| session296/phase4-integrated-dashboard.drawio | Phase 4: SCM全体の一元管理 |

**未完了**:
- スライドへの図組み込み → Session 302で実施予定

**詳細**: [session301/session-summary.md](../session301/session-summary.md)

---

## Session 302 (2026-03-24)

**概要**: 社長説明スライドへの図組み込み・レビュー対応

**重要な成果**:
- Phase 1-4の図をスライドに配置
- 理想像スライドを追加
- 構成見直し（「前回から」→「3月の調査」、ページ数削減）
- 表記修正（仕損、ナカヨ/ワタナベ）
- スタイル調整（フォントサイズ縮小、画像max-height設定）

**作成ファイル**:

| ファイル | 内容 |
|----------|------|
| session302/image.png | 理想像（統合在庫管理システム）のスクショ |

**未完了**:
- 画像サイズの微調整 → Session 303で実施予定

**詳細**: [session302/session-summary.md](../session302/session-summary.md)

---

## Session 303 (2026-03-24)

**概要**: スライドの最終調整・kintoneスライド追加

**重要な成果**:
- kintone契約方式のスライド2ページ追加（NTT東日本承認の話）
- Phase 2スライドを2ページに分割
- タイトル・画像サイズの調整

**詳細**: [session303/session-summary.md](../session303/session-summary.md)

---

## Session 304 (2026-03-25)

**概要**: スライドの修正（石川さんからの指摘対応）

**重要な成果**:
- 個人名を削除（掛川さん、小笠原さん、田原さん → SCM担当等）
- 「サービス残業」表現を削除
- 「毎週日曜日」→「毎週末」に変更

**未完了**:
- 画像差し替え・プレビュー確認 → Session 305で実施予定

**詳細**: [session304/session-summary.md](../session304/session-summary.md)

---

## Session 305 (2026-03-25)

**概要**: スライド確認完了・Linear反映・v4構成検討

**重要な成果**:
- スライドv3の画像差し替え確認完了
- Linear更新: QA-8, QA-10 → Done、QA-13新規作成
- 石川さんレビュー対応のv4構成方針検討

**GENメーカー問い合わせ結果**:
- エクスポートのみ対応（品目マスタ・取引先マスタ）
- 各画面DBアクセス不可、開発は未定

**v4構成の方向性**:
- 起承転結で構成（問題→現状→解決策→効果）
- 定量効果（年間104時間）+ リスク低減（委託先在庫）

**未完了**:
- スライドv4の作成 → Session 306で実施予定

**詳細**: [session305/session-summary.md](../session305/session-summary.md)

---

## Session 306 (2026-03-25)

**概要**: GEN・kintone制約を踏まえたスライドv4作成

**重要な成果**:
- スライドv4作成（14枚 → 8枚に削減）
- 起承転結で構成、フローチャートのみ
- 抽象的な内容に変更（具体策は書かない）
- 補足スライド追加（3月の調査活動 + SIPOC図）

**重要な決定**:
- 担当者責任を避ける前提で制約の伝え方を整理
- kintoneはスライドに入れない（和田さん・宮崎さん担当）

**作成ファイル**:

| ファイル | 内容 |
|----------|------|
| session306/ceo-presentation-v4.md | スライドv4 |
| session306/images/ | 関連画像 |

**詳細**: [session306/session-summary.md](../session306/session-summary.md)

---

## Session 307 (2026-03-25)

**概要**: 状況把握・整理、スライドv5作成

**重要な成果**:
- Linear状況確認（9 issues）
- M1 GNSS状況確認（Session 228以降放置、実装ほぼ完成）
- スライドv5作成（AI化検討の記述を修正）

**スライドv5変更点**:
- AI化検討: 「和田さんが検討継続中」→「品質基準書を作成し、作業効率化を検討中。メーカー2社に和田さんから問い合わせ中」

**作成ファイル**:

| ファイル | 内容 |
|----------|------|
| session307/ceo-presentation-v5.md | スライドv5 |
| session307/status-investigation.md | 調査結果（途中） |
| session307/files-reviewed.md | 確認したファイル一覧 |

**未完了**:
- M1 GNSSのLinear issue化
- DX施策の線引き整理

**詳細**: [session307/session-summary.md](../session307/session-summary.md)

---

## Session 308 (2026-03-25)

**概要**: 石川さんアドバイスを受けてスライドv6の方針決定

**重要な成果**:
- 石川さんアドバイスの整理（104時間の扱い、フレームワーク活用、調査期間明示）
- 過去セッション調査（Session 256-307の課題・方針転換を確認）
- 特性要因図の構造化（4カテゴリ: システム/方法/情報/管理）
- スライドv6の方針決定

**重要な決定**:
- 100時間は書かない（インパクト薄い、突っ込まれる可能性）
- 欠品リスクをフロー図で見せる（定量化できなくてもイメージとして伝わる）
- 「現状把握が完了していない」と正直に伝え、調査期間を明示

**作成ファイル**:

| ファイル | 内容 |
|----------|------|
| session308/slide-v6-policy.md | スライドv6の方針（決定事項・構成案） |

**未完了**:
- 調査期間（〇月まで）の決定
- スライドv6の作成
- 特性要因図・欠品リスクフロー図のdrawio作成

**詳細**: [session308/session-summary.md](../session308/session-summary.md)

---

## Session 309 (2026-03-25)

**概要**: スライドv6の作成（特性要因図・欠品リスクフロー図含む）

**重要な成果**:
- 調査期間を6月末までに決定
- 特性要因図のdrawio作成（4カテゴリで課題を構造化）
- 欠品リスクフロー図のdrawio作成
- スライドv6作成（v5ベースで特性要因図・結論を追加）

**重要な決定**:
- 調査期間: 6月末まで（受入検査は部品点数が多いため）
- 課題は2つに絞らず、特性要因図で全体像を見せる

**作成ファイル**:

| ファイル | 内容 |
|----------|------|
| session309/fishbone-scm-issues.drawio | 特性要因図 |
| session309/stockout-risk-flow.drawio | 欠品リスクフロー図 |
| session309/ceo-presentation-v6.md | スライドv6 |

**未完了**:
- drawioのスクショ撮影・配置

**詳細**: [session309/session-summary.md](../session309/session-summary.md)

---
