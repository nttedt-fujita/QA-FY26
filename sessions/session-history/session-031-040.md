...
**概要**: 方針見直しの具体化（特性要因図 + プロトタイプ設計見直し + 一次調査準備）。

**背景**: Session 39で整理した「m3-research-files」の知見を具体的なアクションに落とし込む。

**作成ファイル**:
| ファイル | 内容 |
|----------|------|
| [fishbone-diagram.md](../session40/fishbone-diagram.md) | **特性要因図**（4M分析、確認済み/仮説を明記） |
| [prototype-redesign.md](../session40/prototype-redesign.md) | **プロトタイプ設計見直し**（Option B: 一次調査優先を推奨） |
| [field-investigation-checklist.md](../session40/field-investigation-checklist.md) | **一次調査チェックリスト**（現場用、P0-P2優先度付き） |
| [session-summary.md](../session40/session-summary.md) | セッションサマリー |

**重要な決定**:
- **プロトタイプは一旦凍結** — 一次調査なしでツールを作っても的外れになる可能性大
- **即効施策を先に** — 抜取検査計画見直し + Googleフォームで即日開始
